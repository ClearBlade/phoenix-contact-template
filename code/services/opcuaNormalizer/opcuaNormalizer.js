function opcuaNormalizer(req, resp) {
  ...
  const OUTGOING_DB_TOPIC_PLATFORM =
    "_dbupdate/_monitor/_asset/<ASSET_ID>/status/_platform";
        ...
        Promise.all([getAssetMap(edgeId), getAssetsByTreeId(asset.tree_id)])
          .then(function (mapAndAssets) {
            ...
            Object.keys(assetMap).forEach(function (key) {
              var customData = {};
              var assetId = assetMap[key].asset_id.replace(
                "{asset_prefix}",
                prefix
              );
              ...
              if (asset !== undefined) {
                ...
                for (var nodeid in msg.data) {
                  if (nodeid !== key) {
                    ...
                    customData[assetMap[key].attribute_id] =
                      msg.data[nodeid].value;
                  }
                }
                ...
                if (existingMsg !== undefined) {
                  existingMsg.custom_data[assetMap[key].attribute_id] =
                    customData[assetMap[key].attribute_id];
                  messagesToPublish[findIndexById(assetId, messagesToPublish)] =
                    existingMsg;
                } else {
            ...
            messagesToPublish.forEach(function (assetModelMsg) {
              log(
                "publishing message to asset/db/history topics: " +
                  JSON.stringify(assetModelMsg)
              );
              cbClient.publish(
                OUTGOING_ASSET_MODEL_TOPIC.replace(
                  "<ASSET_ID>",
                  assetModelMsg.id
                ),
                JSON.stringify(assetModelMsg)
              ...
              cbClient.publish(
                OUTGOING_DB_TOPIC.replace("<ASSET_ID>", assetModelMsg.id),
                JSON.stringify(assetModelMsg)
              );
              cbClient.publish(
                OUTGOING_DB_TOPIC_PLATFORM.replace(
                  "<ASSET_ID>",
                  assetModelMsg.id
                ),
                JSON.stringify(assetModelMsg)
              ...
              cbClient.publish(
                OUTGOING_HISTORY_TOPIC.replace("<ASSET_ID>", assetModelMsg.id),
                JSON.stringify(assetModelMsg)
              );
            });
          })
      ...
      cbTagAttMapCol
        ...
        .then(function (results) {
          if (results.DATA.length >= 1) {
            ...
            reject(
              "getAssetMap: No rows returned from tag_attribute_mapping collection for id: " +
                edgeId
            );
          }
        })
        .catch(function (error) {
          reject(
            "Error retrieving from tag_attribute_mapping collection: " + error
          );
        });
    });
  }
      ...
      cbAssetsCol
        ...
        .then(function (results) {
          if (results.DATA.length >= 1) {
            ...
            reject(
              "getAssetById: No rows returned from Assets collection for id: " +
                id
            );
          }
        })
